...
export def "main" [] {
    ...
    let sqlquery = "SELECT URLTable.OwnerID as RIN, URL, NameTable.Given COLLATE NOCASE as Given, NameTable.Surname COLLATE NOCASE as Surname, 
       NameTable.BirthYear, NameTable.DeathYear, STRFTIME(DATETIME(URLTable.UTCModDate + 2415018.5)) AS LastUpdate 
       FROM URLTable 
